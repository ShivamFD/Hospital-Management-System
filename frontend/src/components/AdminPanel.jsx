...
// Dashboard Layout Components (unchanged)
const DashboardContainer = styled.div`
  display: flex;
  min-height: 100vh;
  background-color: #f5f7fb;
    ...
    <DashboardContainer>
      ...
      <Sidebar>
        ...
        <SidebarMenu>
          ...
          <SidebarMenuItem 
            className={activeSection === 'doctorsList' ? 'active' : ''} 
            onClick={() => setActiveSection('doctorsList')}
          >
            <MenuIcon>👨‍⚕️</MenuIcon> Doctors List
          </SidebarMenuItem>
          <SidebarMenuItem>
            <MenuIcon>🚪</MenuIcon> Logout
          </SidebarMenuItem>
      ...
      <MainContent>
        ...
        {activeSection === 'doctorsList' && (
          <DirectoryCard>
            <DirectoryCardHeader>
              <DirectoryCardTitle>Doctors Directory</DirectoryCardTitle>
            </DirectoryCardHeader>
            
            <DirectorySearchBar>
              <DirectorySearchInput
                type="text"
                placeholder="Search doctors by name, email, or specialist..."
                value={searchQuery}
            ...
            <DirectoryTable>
              <DirectoryTableHead>
                <DirectoryTableRow>
                  <DirectoryTableHeader>Doctor</DirectoryTableHeader>
                  <DirectoryTableHeader>Specialization</DirectoryTableHeader>
                  <DirectoryTableHeader>Actions</DirectoryTableHeader>
                </DirectoryTableRow>
              </DirectoryTableHead>
              <tbody>
                {filteredDoctors.length > 0 ? (
                  filteredDoctors.map((doctor) => (
                    <DirectoryTableRow key={doctor._id}>
                      <DirectoryTableCell>
                        <DirectoryDoctorInfo>
                          <DirectoryAvatarContainer>
                            {doctor.image ? (
                              <DirectoryAvatarImage src={`http://localhost:5000/${doctor.image}`} alt={doctor.name} />
                            ) : (
                              <DirectoryAvatarInitials>
                                {getInitials(doctor.name)}
                              </DirectoryAvatarInitials>
                            )}
                          </DirectoryAvatarContainer>
                          <DirectoryDoctorDetails>
                            <DirectoryDoctorName>{doctor.name || 'Unknown Doctor'}</DirectoryDoctorName>
                            <DirectoryDoctorEmail>{doctor.email || 'No email'}</DirectoryDoctorEmail>
                          </DirectoryDoctorDetails>
                        </DirectoryDoctorInfo>
                      </DirectoryTableCell>
                      <DirectoryTableCell>
                        <DirectorySpecialistBadge>
                          {doctor.specialist || 'Not specified'}
                        </DirectorySpecialistBadge>
                      </DirectoryTableCell>
                      <DirectoryTableCell>
                        <DirectoryActionButton className="edit">
                          Edit
                        </DirectoryActionButton>
                        <DirectoryActionButton className="delete">
                          Delete
                        </DirectoryActionButton>
                      </DirectoryTableCell>
                    </DirectoryTableRow>
                  ...
                  <DirectoryTableRow>
                    <DirectoryTableCell colSpan="3">No doctors available</DirectoryTableCell>
                  </DirectoryTableRow>
                )}
              </tbody>
            </DirectoryTable>
